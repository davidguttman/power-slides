...
- \`quote\` — quote/text plus optional image column. Fields: \`quote\`/\`text\`, \`eyebrow\`, \`image\`/\`img\`/\`src\`, \`background\`, \`brightness\`, \`font\`, \`color\`, \`size\`, layout knobs \`columns\`/\`gridTemplateColumns\`, \`gap\`, \`padding\`, \`alignItems\`, \`justifyItems\`, copy knobs \`align\`/\`copyAlign\`, \`copyJustify\`, \`copyAlignSelf\`, \`copyMaxWidth\`, \`copyStyle\`, and image knobs \`fit\`, \`maxHeight\`/\`imageMaxHeight\`, \`maxWidth\`/\`imageMaxWidth\`, \`radius\`, \`shadow\`, \`imageAlign\`, \`imageJustify\`, \`imageAlignSelf\`, \`imageJustifySelf\`, \`mediaStyle\`, \`imageStyle\`.
