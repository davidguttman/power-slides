...
assert.strictEqual(initializedSlides[1].columns[0].image, '/github-render.png', 'starter second slide uses a copied generated image asset inside a designed columns moment')
assert.strictEqual(initializedSlides[1].columns[1].title, 'Static assets are first-class.', 'starter second slide explains static assets as a purposeful story beat')
assert(initializedSlides[1].columns[1].bullets.includes('Image primitives work inside designed layouts'), 'starter second slide keeps image primitive guidance in the story')
...
assert.strictEqual(initializedSlides[3].columns[1].subtitle, 'Every column is another slide.', 'starter columns gives the first composition idea its own moment')
assert.strictEqual(initializedSlides[3].columns[1].bullets, undefined, 'starter composition column does not render the old bullet list')
assert.strictEqual(initializedSlides[4].title, 'Mix assets, copy, media, and embeds without glue code.', 'starter fifth slide separates the mixed-media composition idea')
assert.strictEqual(initializedSlides[5].title, 'Keep the story readable while the layout gets richer.', 'starter sixth slide separates the readability composition idea')
...
import(path.join(root, 'index.mjs')).then(async mod => {
  ...
  try {
    ...
    assert.strictEqual(mod.inferSlideType(showcaseSlides[1]), 'columns', 'showcase story 2 is a designed static-assets columns moment')
    assert.strictEqual(showcaseSlides[1].columns[0].image, '/github-render.png', 'showcase story 2 demonstrates the image primitive with purpose')
    assert.strictEqual(showcaseSlides[1].columns[1].title, 'Static assets are first-class.', 'showcase story 2 explains static assets')
    ...
    assert.strictEqual(showcaseSlides[3].columns[1].bullets, undefined, 'showcase composition column does not render the old bullet list')
    assert.strictEqual(mod.inferSlideType(showcaseSlides[4]), 'text', 'showcase shape 5 is the mixed-assets story moment')
    assert.strictEqual(mod.inferSlideType(showcaseSlides[5]), 'text', 'showcase shape 6 is the readability story moment')
