...
import(path.join(root, 'index.mjs')).then(async mod => {
  ...
  try {
    ...
    assert.strictEqual(phoneViewport.style.overflow, 'hidden', 'phone-framed iframe viewport clips cover-scaled overflow')
    assert.strictEqual(phoneViewport.style.display, 'block', 'phone-framed iframe viewport avoids inline layout artifacts')
    assert.strictEqual(phoneViewport.style.lineHeight, 0, 'phone-framed iframe viewport suppresses baseline gaps')
    assert.strictEqual(phoneViewport.style.transform, 'translate(-50%, -50%) scale(max(calc(100cqw / 390px), calc(100cqh / 844px)))', 'phone-framed iframe viewport cover-fills the rounded screen')
    assert.strictEqual(phoneFrame.style.width, 'calc(390px + 18px)', 'phone-framed iframe extends under the clipped scrollbar gutter')
    assert.strictEqual(phoneFrame.style.height, '844px', 'phone-framed iframe gets logical phone viewport height')
    assert.strictEqual(phoneFrame.style.display, 'block', 'phone-framed iframe avoids inline baseline artifacts')
    assert.strictEqual(phoneFrame.style.scrollbarWidth, 'none', 'phone-framed iframe asks supporting engines to hide scrollbar chrome')
    ...
    assert.strictEqual(columnIframeViewport.style.overflow, 'hidden', 'columns phone iframe viewport clips cover-scaled overflow')
    assert.strictEqual(columnIframeViewport.style.transform, 'translate(-50%, -50%) scale(max(calc(100cqw / 390px), calc(100cqh / 844px)))', 'columns phone iframe viewport cover-fills the rounded screen')
    assert.strictEqual(columnIframeFrame.style.width, 'calc(390px + 18px)', 'columns phone iframe extends under the clipped scrollbar gutter')
    assert.strictEqual(columnIframeFrame.style.height, '844px', 'columns phone iframe gets logical phone viewport height')
    assert.strictEqual(columnIframeFrame.style.display, 'block', 'columns phone iframe avoids inline baseline artifacts')
